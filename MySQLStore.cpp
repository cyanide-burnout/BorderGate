  ...
  onReport(NULL)
{
  const char* queries[] =
  {
    ...
    "  WHERE `Address` = ?",
    // VERIFY_REPEATER
    "SELECT COUNT(Call2) FROM Table1"
    "  WHERE Call1 = ?",
    // RESET_USER_STATE
...
bool MySQLStore::execute(int index, int count1, int count2, ...)
{
      ...
      (columns    && mysql_stmt_store_result(statement)))
  {
    ...
    report(LOG_CRIT, "MySQL error while processing statement %d: %s\n", index, error);
    result = false;
  }
...
bool MySQLStore::findRoute(const char* call, struct DStarRoute* route, struct in_addr* address)
{
  ...
  if (execute(FIND_ROUTE, 1, 4,
        MYSQL_TYPE_STRING, filter, LONG_CALLSIGN_LENGTH,
        MYSQL_TYPE_STRING, route->yourCall, LONG_CALLSIGN_LENGTH,
...
bool MySQLStore::verifyRepeater(const char* call)
{
  int outcome = 0;
