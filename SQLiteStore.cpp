...
#define VERIFY_REPEATER         2
#define RESET_USER_STATE        3
#define STORE_USER_SERVER       4
#define STORE_USER              5
#define REMOVE_USER             6
#define FIND_ACTIVE_BOT         7
#define GET_LAST_MODIFIED_DATE  8
#define GET_COUNT_BY_DATE       9
#define STORE_TABLE_DATA        10
  ...
  onReport(NULL)
{
  ...
  {
    // FIND_ROUTE
    "SELECT Station, Repeater, Gateway, Address FROM Routes"
    "  WHERE Call = ?;",
    // VERIFY_ADDRESS
...
bool SQLiteStore::findRoute(const char* call, struct DStarRoute* route, struct in_addr* address)
{
  ...
  if (sqlite3_bind_text(statement, 1, filter, LONG_CALLSIGN_LENGTH, SQLITE_STATIC) == SQLITE_OK)
  {
    if (sqlite3_step(statement) == SQLITE_ROW)
    {
      ...
      CopyDStarCall((char*)sqlite3_column_text(statement, 0), route->yourCall, NULL, 0);
      CopyDStarCall((char*)sqlite3_column_text(statement, 1), route->repeater2, NULL, 0);
      CopyDStarCall((char*)sqlite3_column_text(statement, 2), route->repeater1, NULL, 0);
      inet_aton((char*)sqlite3_column_text(statement, 3), address);
...
bool SQLiteStore::verifyRepeater(const char* call)
{
  bool outcome = false;
  char filter[LONG_CALLSIGN_LENGTH + 1];
  CopyDStarCall(call, filter, NULL, COPY_MAKE_STRING);

  sqlite3_stmt* statement = statements[VERIFY_REPEATER];
